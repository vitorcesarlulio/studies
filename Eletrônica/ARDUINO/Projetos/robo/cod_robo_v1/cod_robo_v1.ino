...
//Definindo os pinos do sensor ultrasonico
...
#define enA  11        //pino enA na porta digital 10 //esse cara que controla a velocidade
...
#define enB  5         //pino enB na porta digital 5  //esse cara que controla a velocidade
#define in3  7         //pino in3 na porta digital 7
#define in4  6         //pino in4 na porta digital 6

Servo servoSensor;       // Crie um objeto Servo para controlar o Servo. //chamndo a bib e dando um nome a ela

//função para procurar obtasculo a todo o tempo
int Procurar (void) {
  float duracao = 0.0;              // variavael para guartar a duração do retorno do som
  float CM = 0.0;                   // variavael para guartar a distancia
...
int velocidadeMotorUm = 148; //olhando de frente para o sensor é o motor da direita
int velocidadeMotorDois = 150; //esquerdo
...
void loop() {
  ...
  Distancia = Procurar ();                          // Medindo a Distancia em CM.
   if (Distancia < 35) {                             // Se há obstáculo encontrado a menos de 40cm. // DA PRA COLOCAR MENOS
    direcao ();                                      // Se Frente estiver bloqueado, mude de direção
   }
   else if (Distancia >= 35)  {                      // Se o obstáculo for encontrado entre a mais de 40cm
